...
import com.android.ks.tool.IXposedHookZygoteInit;
...
import com.android.ks.tool.ShadowConnect.CopyOnWriteSortedSet;
import com.android.ks.tool.callbacks.XC_LayoutInflated;
import com.android.ks.tool.callbacks.XC_LayoutInflated.LayoutInflatedParam;
import com.android.ks.tool.callbacks.XCallback;
import xposed.dummy.XResourcesSuperClass;
import xposed.dummy.XTypedArraySuperClass;

import static com.android.ks.tool.ShadowHelpers.decrementMethodDepth;
...
import static com.android.ks.tool.ShadowHelpers.getLongField;
import static com.android.ks.tool.ShadowHelpers.getObjectField;
import static com.android.ks.tool.ShadowHelpers.incrementMethodDepth;
...
//			Log.w(XposedBridge.TAG, "Package name for " + resDir + " had to be retrieved via parser");
			...
			if (incrementMethodDepth("getDrawable") == 1) {
				...
				if (replacement instanceof DrawableLoader) {
					try {
						...
						if (result != null)
							return result;
					} catch (Throwable t) { ShadowConnect.log(t); }
				} else if (replacement instanceof Integer) {
			...
			if (callbacks != null) {
				...
				if (value.type == TypedValue.TYPE_STRING) {
					...
					if (components.length == 3)
						...
						ShadowConnect.log("Unexpected resource path \"" + value.string.toString()
								+ "\" for resource id 0x" + Integer.toHexString(id));
				} else {
					ShadowConnect.log(new NotFoundException("Could not find file name for resource id 0x") + Integer.toHexString(id));
				}
	...
	private static int translateResId(int id, XResources origRes, Resources repRes) {
		try {
			...
			if (!repResDefined && origResId == 0 && !entryType.equals("id")) {
				ShadowConnect.log(entryType + "/" + entryName + " is neither defined in module nor in original resources");
				return 0;
			}
	...
	private static int translateAttrId(String attrName, XResources origRes) {
		...
		try {
			...
			ShadowConnect.log("Attribute " + attrName + " not found in original resources");
		}
		return origAttrId;
	}
