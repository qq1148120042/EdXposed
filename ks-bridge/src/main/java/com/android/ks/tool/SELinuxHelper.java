...
import com.android.ks.tool.services.BaseService;
import com.android.ks.tool.services.DirectAccessService;
			...
			try {
				...
				switch (status) {
					...
					case 48:
						...
						break;
					default:
						ShadowConnect.log("Unexpected byte " + status + " in /sys/fs/selinux/enforce");
				}
				fis.close();
			} catch (IOException e) {
				if (e.getMessage().contains("Permission denied")) {
					...
					ShadowConnect.log("Failed to read SELinux status: " + e.getMessage());
					result = false;
				}
			}
		}
