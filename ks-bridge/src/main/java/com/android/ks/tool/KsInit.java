...
import com.android.ks.tool.callbacks.XC_InitPackageResources;
import com.android.ks.tool.callbacks.XC_InitZygote;
import com.android.ks.tool.callbacks.XC_LoadPackage;
import com.android.ks.tool.callbacks.XCallback;

import static com.android.ks.tool.ShadowConnect.hookAllConstructors;
import static com.android.ks.tool.ShadowConnect.hookAllMethods;
import static com.android.ks.tool.ShadowConnect.sInitPackageResourcesCallbacks;
import static com.android.ks.tool.ShadowConnect.sInitZygoteCallbacks;
import static com.android.ks.tool.ShadowConnect.sLoadedPackageCallbacks;
import static com.android.ks.tool.ShadowHelpers.callMethod;
import static com.android.ks.tool.ShadowHelpers.closeSilently;
import static com.android.ks.tool.ShadowHelpers.findAndHookMethod;
import static com.android.ks.tool.ShadowHelpers.findClass;
import static com.android.ks.tool.ShadowHelpers.findFieldIfExists;
import static com.android.ks.tool.ShadowHelpers.getObjectField;
import static com.android.ks.tool.ShadowHelpers.getParameterIndexByType;
import static com.android.ks.tool.ShadowHelpers.setStaticBooleanField;
import static com.android.ks.tool.ShadowHelpers.setStaticLongField;
import static com.android.ks.tool.ShadowHelpers.setStaticObjectField;

public final class KsInit {
    private static final String TAG = ShadowConnect.TAG;
    ...
    public static void initForZygote(boolean isSystem) throws Throwable {
        ...
        if (findFieldIfExists(ZygoteInit.class, "BOOT_START_TIME") != null) {
            setStaticLongField(ZygoteInit.class, "BOOT_START_TIME", ShadowConnect.BOOT_START_TIME);
        }
        // Samsung
    ...
    private static void hookResources() throws Throwable {
        if (!KsConfigGlobal.getConfig().isResourcesHookEnabled() || disableResources) {
            return;
        }

        if (!KsConfigGlobal.getHookProvider().initXResourcesNative()) {
...
//        });
        ...
        findAndHookMethod(TypedArray.class, "obtain", Resources.class, int.class,
                new XC_MethodHook() {
                    ...
                    protected void endRunning(MethodHookParam param) throws Throwable {
                        ...
                        Method resizeMethod = ShadowHelpers.findMethodBestMatch(
        ...
        XResources systemRes = new XResources(
                (ClassLoader) ShadowHelpers.getObjectField(Resources.getSystem(), "mClassLoader"));
        systemRes.setImpl((ResourcesImpl) ShadowHelpers.getObjectField(Resources.getSystem(), "mResourcesImpl"));
    ...
    private static XResources cloneToXResources(XC_MethodHook.MethodHookParam param, String resDir) {
        ...
        XResources newRes = new XResources(
                (ClassLoader) ShadowHelpers.getObjectField(param.getResult(), "mClassLoader"));
        newRes.setImpl((ResourcesImpl) ShadowHelpers.getObjectField(param.getResult(), "mResourcesImpl"));
        newRes.initObject(resDir);

        // Invoke handleInitPackageResources().
        if (newRes.isFirstLoad()) {
            String packageName = newRes.getPackageName();
            XC_InitPackageResources.InitPackageResourcesParam resparam = new XC_InitPackageResources.InitPackageResourcesParam(ShadowConnect.sInitPackageResourcesCallbacks);
    ...
    public static boolean loadModules(boolean callInitZygote) throws IOException {
        ...
        synchronized (moduleLoadLock) {
            ClassLoader topClassLoader = ShadowConnect.BOOTCLASSLOADER;
            ClassLoader parent;
            while ((parent = topClassLoader.getParent()) != null) {
                topClassLoader = parent;
            }

            String moduleList = KsConfigGlobal.getConfig().getModulesList();
    ...
    private static boolean loadModule(String apk, ClassLoader topClassLoader, boolean callInitZygote) {
        ...
        try {
            if (mcl.loadClass(ShadowConnect.class.getName()) != null) {
        ...
        try {
            ...
            parentField.set(mcl, KsInit.class.getClassLoader());
        ...
        try {
            ...
            ZipEntry zipEntry = zipFile.getEntry("assets/ks_init");
            if (zipEntry == null) {
                Log.e(TAG, "  assets/ks_init not found in the APK");
                closeSilently(zipFile);
                return false;
            }
            is = zipFile.getInputStream(zipEntry);
        } catch (IOException e) {
            Log.e(TAG, "  Cannot read assets/ks_init in the APK", e);
            closeSilently(zipFile);
            return false;
        }
        ...
        try {
            ...
            while ((moduleClassName = moduleClassesReader.readLine()) != null) {
                ...
                try {
                    ...
                    if (ShadowConnect.isZygote) {
                        if (moduleInstance instanceof IXposedHookZygoteInit) {
                            IXposedHookZygoteInit.StartupParam param = new IXposedHookZygoteInit.StartupParam();
                            param.modulePath = apk;
                            param.startsSystemServer = startsSystemServer;

                            ShadowConnect.hookInitZygote(new IXposedHookZygoteInit.Wrapper(
                        ...
                        if (moduleInstance instanceof IXposedHookLoadPackage)
                            ShadowConnect.hookLoadPackage(new IXposedHookLoadPackage.Wrapper(
                                    (IXposedHookLoadPackage) moduleInstance, apk));

                        if (moduleInstance instanceof IXposedHookInitPackageResources)
                            ShadowConnect.hookInitPackageResources(new IXposedHookInitPackageResources.Wrapper(
