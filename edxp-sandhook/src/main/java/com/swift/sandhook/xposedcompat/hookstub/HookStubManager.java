...
public class HookStubManager {
    ...
    public static ShadowConnect.AdditionalHookInfo[] additionalHookInfos;

    static {
        is64Bit = SandHook.is64Bit();
        Class stubClass = is64Bit ? MethodHookerStubs64.class : MethodHookerStubs32.class;
        stubSizes = (int[]) ShadowHelpers.getStaticObjectField(stubClass, "stubSizes");
        Boolean hasBackup = (Boolean) ShadowHelpers.getStaticObjectField(stubClass, "hasStubBackup");
        ...
        if (stubSizes != null && stubSizes.length > 0) {
            ...
            additionalHookInfos = new ShadowConnect.AdditionalHookInfo[ALL_STUB];
        }
    }


    public static HookMethodEntity getHookMethodEntity(Member origin, ShadowConnect.AdditionalHookInfo additionalHookInfo) {
    ...
    public static long hookBridge(int id, CallOriginCallBack callOrigin, long... stubArgs) throws Throwable {
        ...
        do {
            try {
                ((XC_MethodHook) snapshot[beforeIdx]).callbeginRunning(param);
            } catch (Throwable t) {
                // reset result (ignoring what the unexpectedly exiting callback did)
                if( BuildConfig.DEBUG ) ShadowConnect.log(t);
        ...
        if (!param.returnEarly) {
            try {
                ...
                if( BuildConfig.DEBUG ) ShadowConnect.log(e);
        ...
        do {
            ...
            try {
                ((XC_MethodHook) snapshot[afterIdx]).callendRunning(param);
            } catch (Throwable t) {
                if( BuildConfig.DEBUG ) ShadowConnect.log(t);
    ...
    public static Object hookBridge(Member origin, Method backup, ShadowConnect.AdditionalHookInfo additionalHookInfo, Object thiz, Object... args) throws Throwable {
