...
if [[ ${BOOTMODE} == true ]]; then
  [[ "$(pm path org.ks.tool.manager)" == "" && "$(pm path com.android.ks.tool.installer)" == "" ]] && NO_MANAGER=true
fi
...
if [[ -f /data/adb/edxp/misc_path ]]; then
  ...
  if [[ -d /data/user_de/0/org.ks.tool.manager/conf/ ]]; then
    mkdir -p /data/misc/$MISC_PATH/0/conf
    cp -r /data/user_de/0/org.ks.tool.manager/conf/* /data/misc/$MISC_PATH/0/conf/
